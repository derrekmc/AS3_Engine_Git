...
package {
    ...
    public class ImageLoader extends MovieClip{
		...
		[Embed(source = "pak/invisible.png")]
        private var invisibleImage:Class;
		...
		public function completeHandler(event:Event):void {
           ...
           // Debug.print("Loading completeHandler", 4);
			...
			for(var j=0; j < loader_buffer_requests.length; j++){
				...
				if(loader_buffer_requests[j] == this.url){
					
					for(var i=0; i < loader_buffer_content.length; i++){
						
						if(loader_buffer_content[i].url == loader_buffer_requests[j] && loader_buffer_content[i].completed != true){
							...
							loader_buffer_content[i].content = event.target;
							loader_buffer_content[i].completed = true;
							loader_buffer_content[i].dispatchEvent(new Event("loading_complete", true));
							//trace("dispatching koad");
							
							//trace("loader_buffer_content working::" + loader_buffer_content[i].url, loader_buffer_requests[j], this.url, event.target.url);
						}
		...
		public function workingh(e:Event){
			//Display.addchild(e.target.content);
			trace("workinf: " + e.target.name);
		}
